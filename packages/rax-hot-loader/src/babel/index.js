...
module.exports = function plugin(args) {
  ...
  if (this && this.callback) {
    throw new Error(
      'Rax Hot Loader: You are erroneously trying to use a Babel plugin ' +
        'as a Webpack loader. We recommend that you use Babel, ' +
        'remove "rax-hot-loader/babel" from the "loaders" section ' +
        ...
        classBody.get('body').forEach(path => {
          if (path.isClassProperty()) {
            ...
            if (node.value && node.value.type === 'ArrowFunctionExpression') {
              ...
              const newIdentifier = t.identifier(`__${node.key.name}__RAX_HOT_LOADER__`);
